...
def getOptions():
    ...
    userOptions["Test StringList"]["values"] = ["Option 1", "Option 2", "Option 3"]
...
def generateInput():
    ...
    # listed in the array:
    files = []
    files.append({"filename": f"{baseName}.opts", "contents": inp})
    files.append({"filename": f"{baseName}.testFilePath", "filePath": filePath})
    ...
    result["mainFile"] = f"{baseName}.opts"

    result["warnings"] = []
    for i in range(numWarnings):
        result["warnings"].append(f"Warning number {i + 1:d}...")
